...
# --- Self-contained model, parameterized by num_subbands and stem_channels ---
...
def per_snr_low_only():
    """Only the low-SNR bins -- this is a quick screen, not a full report."""
    model.eval()
    correct, total = {}, {}
    with torch.no_grad():
...
def run_config(name, num_subbands, stem_channels, patience=8, max_epochs=50):
    ...
    for epoch in range(max_epochs):
        ...
        band_loss, band_acc = evaluate_val()
        ...
        if (epoch + 1) % 5 == 0 or no_improve >= patience:
            print(f"  Epoch {epoch+1:2d}: Band Acc {band_acc:.2f}%, no_improve={no_improve}")
    ...
    low_snr = per_snr_low_only()
    print(f"  >> Test Acc: {test_acc:.2f}%  |  params: {params:,}")
    return {'name': name, 'params': params, 'test_acc': test_acc, 'low_snr': low_snr}


configs = [
    ("original",     8, 64),
    ("backend_cut",  8, 32),   # tests: does shrinking generic backend hurt?
    ("frontend_cut", 4, 64),   # tests: does shrinking the front-end hurt more?
]

results = [run_config(*c) for c in configs]

print(f"\n{'='*55}\nSCREEN SUMMARY\n{'='*55}")
print(f"{'Config':<14}{'Params':>9}{'TestAcc':>10}")
for r in results:
    print(f"{r['name']:<14}{r['params']:>9,}{r['test_acc']:>9.2f}%")

snrs = sorted(results[0]['low_snr'].keys())
print(f"\n{'Config':<14}" + "".join(f"{s:>7}" for s in snrs))
for r in results:
    print(f"{r['name']:<14}" + "".join(f"{r['low_snr'][s]:>6.1f}%" for s in snrs))